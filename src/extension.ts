...
export function activate(context: vscode.ExtensionContext) {
  ...
  const error_color =
    vscode.workspace.getConfiguration("indentRainbow")["errorColor"] || "rgba(128,32,32,0.3)";
  ...
  const ignoreLinePatterns =
    vscode.workspace.getConfiguration("indentRainbow")["ignoreLinePatterns"] || [];
  const colorOnWhiteSpaceOnly =
    vscode.workspace.getConfiguration("indentRainbow")["colorOnWhiteSpaceOnly"] || false;
  const indicatorStyle =
    vscode.workspace.getConfiguration("indentRainbow")["indicatorStyle"] || "classic";
  const lightIndicatorStyleLineWidth =
    vscode.workspace.getConfiguration("indentRainbow")["lightIndicatorStyleLineWidth"] || 1;
